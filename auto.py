...
class pic(tk.Frame):
    ...
    home=[0.96,0.04]
    keyantubiao=[0.58,0.96]
    junbu=[0.32,0.5]
    if_run=False
    zuo_biao=np.zeros((10,2),dtype=np.int32)
    cishu=[0]*10  #第0个为截屏次数 #第一个为是否开始，开始后变为1 第三个为边框截图
    start_time=0#时间戳
    rest_time=0
    ration=1
    if_tiaozhuan2=False
    ...
    def __init__(self,master=None):
        ...
        def biankuangjietu():
            ...
            self.screenshot(3)
            cv2.namedWindow("123")
            a=cv2.imread("p/3.png")
            b=a.copy()
            #a=cv2.resize(a,None,None,0.7,0.7)
            cv2.setMouseCallback("123",myevent)
            while self.cishu[2]!=2:
                cv2.imshow("123",a)
                cv2.waitKey(100)
                if self.cishu[2]==1:
                    cv2.circle(a,self.zuo_biao[2],2,(255,0,255),-1,-1)
                if self.cishu[2]==2:
                    cv2.rectangle(b,self.zuo_biao[2],self.zuo_biao[3],(255,0,255),1)
                    cv2.imshow("123",b)
                    cv2.waitKey(1000)
                    cv2.destroyWindow("123")
        master.title("碧蓝程序")
        ...
        def gotoweituo():
            if self.if_tiaozhuan2:
                ...
                pyautogui.click()
                pyautogui.click()
                quick_move(self.home[0]*0.92,self.home[1]*3.5)
                pyautogui.click()
                quick_move(self.home[0],self.home[1])
                pyautogui.click()
                pyautogui.click()
                pyautogui.click()
                quick_move(0.02,0.22)
                pyautogui.click()
                quick_move(0.345,0.383)
                pyautogui.click()
                quick_move(self.home[0],self.home[1])
                for i in range(10):
                    time.sleep(0.2)
        ...
        def tiaozhuan():
            if(self.if_tiaozhuan):
                ...
                time.sleep(1)
                wt=self.get_time()
                if(wt[0]==0 and wt[1]==0 and wt[5]==0 and wt[2]==0 and wt[3]==0 and wt[4]==0):
                    ...
                    if(self.weituowancheng):
                        if(self.if_keyan==1 and self.if_weituo==1):
                            if(nowtime>2000):
                                ...
                                self.set_weituo_time([0,0,3,0,0,0])
        ...
        s8=tk.Button(master,text="截取模拟器边框（非必要）",width=20,command=biankuangjietu)
        s8.place(relx=0.3,rely=0.7)
        def jixu():
            self.if_tiaozhuan=True
    ...
    def zhuhanshu(self):
        if self.cishu[1]==0:
            ...
            return
    def process(self,img1,background):
        
        heisebj=cv2.bitwise_xor(img1,background)
        heisebj=cv2.cvtColor(heisebj,cv2.COLOR_BGR2GRAY)
        img=cv2.bitwise_and(img1,img1,mask=heisebj)
        img=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        r,img=cv2.threshold(img,30,255,cv2.THRESH_BINARY)
        #k=np.ones((3,3),dtype=np.int8)
        #img=cv2.morphologyEx(img,cv2.MORPH_OPEN,k,iterations=1) 
        #img=cv2.GaussianBlur(img,(7,7),0)
        #cv2.imshow("Fsfas",img)
        #cv2.waitKey(0)
        ...
        for i in range(len(contours)):
            if(cv2.arcLength(contours[i],True)>maxlen):
                maxlen=cv2.arcLength(contours[i],True)
                maxi=i
        #print(maxi,maxlen)
        #print(contours[maxi])
        #appro=cv2.approxPolyDP(contours[maxi],0.02*maxlen,True)
        cv2.drawContours(zeros,contours[maxi],-1,(255,255,255),1)
        #cv2.imshow("Fsfas",zeros)
        #cv2.waitKey(0)
        zbdian=[]
        for i in range(img.shape[0]):
            for j in range(img.shape[1]):
                if(zeros.item(i,j)==255):
                    zbdian.append(i)
                    zbdian.append(j)
        jishu=zbdian[::2]
        x1=max(set(jishu),key=jishu.count)
        while x1 in jishu:
            jishu.remove(x1)
        x2=max(set(jishu),key=jishu.count)
        oushu=zbdian[1::2]
        y1=max(set(oushu),key=oushu.count)
        while y1 in oushu:
            oushu.remove(y1)
        y2=max(set(oushu),key=oushu.count)
        first=(min(y1,y2),min(x1,x2))
        second=(max(y1,y2),max(x1,x2))
        global chuang_kou_zuo_biao
        chuang_kou_zuo_biao[0]=first[0]
        chuang_kou_zuo_biao[1]=first[1]
        chuang_kou_zuo_biao[2]=second[0]-first[0]
        chuang_kou_zuo_biao[3]=second[1]-first[1]
        self.quicktanchuang()
    ...
    def movetomoniqi(self):
        ...
        x,y=min_location
        self.moniqix=x+tubiaow//2
        self.moniqiy=y+tubiaoh//2
        self.quicktanchuang()
        
        self.screenshot("first2")
        moniqi=cv2.imread("p/first2.png")
        self.quicktanchuang()
        cv2.waitKey(300)
        self.screenshot("second")
        background=cv2.imread("p/second.png")
        if(self.cishu[2]!=2):
            self.process(moniqi,background)
        else:
            global chuang_kou_zuo_biao
            chuang_kou_zuo_biao[0]=self.zuo_biao[2][0]
            chuang_kou_zuo_biao[1]=self.zuo_biao[2][1]
            chuang_kou_zuo_biao[2]=self.zuo_biao[3][0]-self.zuo_biao[2][0]
            chuang_kou_zuo_biao[3]=self.zuo_biao[3][1]-self.zuo_biao[2][1]
            self.quicktanchuang()
    ...
    def movetokeyan(self):
        ...
        quick_move(self.home[0],self.home[1])
        pyautogui.click()
        quick_move(self.home[0]*0.95,self.home[1]*2.5)
        pyautogui.click()
        quick_move(self.home[0]*0.97,self.home[1]*2)
        pyautogui.click()
        quick_move(self.home[0],self.home[1])
        pyautogui.click()
        pyautogui.click()
    ...
    def get_time(self):
        try:
            ...
            roi=third[realy(0.365):realy(0.475),realx(0.6):realx(0.73)]
            self.get_ration(roi)
            ...
            for i in range(len(lix)//2):
                ...
                for j in range(10):
                    ...
                    cra,shu=cv2.threshold(shu,220,255,cv2.THRESH_BINARY)
                    #cv2.imshow("faa",shu)
                    #cv2.imshow("faas",num)
                    #cv2.waitKey(0)
    ...
    def shibie(self):
        ...
        roi=roi[realy(0.15):realy(0.20),realx(0.28):realx(0.348)]
        r,roi=cv2.threshold(roi,200,255,cv2.THRESH_BINARY)
        ...
        while i < len(listy):
            if listy[i]<avn:
                ...
                i+=1
        roi=cv2.imread("p/xmm.png",0)
        roi=roi[realy(0.13):realy(0.21),realx(0.22):realx(0.36)]
        r,roi=cv2.threshold(roi,200,255,cv2.THRESH_BINARY)
        roi=cv2.copyMakeBorder(roi,10,10,30,30,cv2.BORDER_CONSTANT,value=0)
        #cv2.imshow("fsf",roi)
        #cv2.waitKey(0)
        ...
        if name=="定向研发":
            ...
            roi=tuzhi[realy(0.608):realy(0.742),realx(0.292):realx(0.36)]
        ...
        print("上个名称识别错误")
        return 0
    def quicktanchuang(self):
        pyautogui.moveTo(self.moniqix,self.moniqiy,0.4)
        pyautogui.click()
